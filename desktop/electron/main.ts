...
const runtimeChildren = new Map<string, ChildProcess>()
...
function loadState(options: { normalizeRuntimeState?: boolean } = {}): PersistedState {
  try {
...
function upsertWorkspace(state: PersistedState, record: WorkspaceRecord) {
...
async function choosePort(start: number, usedPorts: number[]): Promise<number> {
...
function spawnCaoServer(command: string, port: number, dataDir: string, workspacePath: string): CaoServerProcess {
...
async function cleanupAll() {
  ...
  if (!state.settings.cleanupOnExit) return
...
async function finishWorkspaceStartup(id: string, port: number, server: CaoServerProcess) {
...
function failWorkspaceStartup(id: string, error: unknown) {
  const state = loadState()
  const workspace = state.workspaces.find((item) => item.id === id)
  if (!workspace) return
...
ipcMain.handle('open-workspace', async (_event, rawPath: string) => {
...
ipcMain.handle('close-workspace', async (_event, id: string) => {
  const state = loadState()
  await cleanupWorkspace(state, id)
  state.workspaces = state.workspaces.map((workspace) =>
    workspace.id === id
...
ipcMain.handle('forget-workspace', async (_event, id: string) => {
  const state = loadState()
  await cleanupWorkspace(state, id)
...
ipcMain.handle('update-agent-status', (_event, workspaceId: string, terminalId: string, status: string) => {
  const state = loadState()
  const workspace = state.workspaces.find((item) => item.id === workspaceId)
...
app.on('before-quit', (event) => {
  if (runtimeChildren.size === 0) return
